"""
Напишите скрипт с графическим интерфейсом пользователя для демонстрации работы класса StringFormatter. 
Примеры окон приведены на рис. 4 (все элементы управления необходимо обязательно реализовать те же, что присутствуют на рисунке). 
Разные комбинации отмеченных чекбоксов приводят к разным цепочкам операций форматирования задаваемой в верхнем поле строки с разными результатами
"""
    ...
    def on_format_click(self, event):
        ...
        if self.sort_checkbox.IsChecked():
            if self.radio_by_size.GetValue():
                result = StringFormatter.sort_by_size(result)
            else:
                result = StringFormatter.sort_by_lec(result)

        if self.clean_checkbox.IsChecked():
            result = StringFormatter.clean(result, int(self.size_spin.Value))

        if self.replace_checkbox.IsChecked():
            result = StringFormatter.hide_figures(result)

        if self.insert_spaces_checkbox.IsChecked():
            result = StringFormatter.add_spases(result)
